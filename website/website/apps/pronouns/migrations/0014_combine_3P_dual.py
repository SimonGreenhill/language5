...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for alignment in ["A", "S", "O", "P"]:
            ...
            # 1. get 3p dual g1 and g2
            qset = orm.PronounType.objects.filter(alignment=alignment, person=person, number=number)
            pt_3pdual_g1 = qset.filter(gender="M").get()
            pt_3pdual_g2 = qset.filter(gender="N").get()
            
            
            # 2. set gendered versions to late sequence
            seqid = pt_3pdual_g1.sequence
            pt_3pdual_g1.sequence = pt_3pdual_g1.sequence + 1000
            pt_3pdual_g1.save()
            print u"Resequenced %s" % pt_3pdual_g1
            pt_3pdual_g2.sequence = pt_3pdual_g2.sequence + 1000
            pt_3pdual_g2.save()
            print u"Resequenced %s" % pt_3pdual_g2
            ...
            for pdm in orm.Paradigm.objects.all():
                ...
                items1 = orm.Pronoun.objects.filter(paradigm=pdm, pronountype=pt_3pdual_g1).get()
                items2 = orm.Pronoun.objects.filter(paradigm=pdm, pronountype=pt_3pdual_g2).get()
            ...
            pt_3pdual_g1.active = False
            print u"Deactivated %s" % pt_3pdual_g1
            pt_3pdual_g2.active = False
            print u"Deactivated %s" % pt_3pdual_g2
