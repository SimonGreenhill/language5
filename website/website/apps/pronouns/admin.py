...
from website.apps.pronouns.models import Paradigm, Pronoun, PronounType
...
class ParadigmAdmin(TrackedModelAdmin, VersionAdmin):
    ...
    search_fields = (
        'label', 'comment',
        'language__language', 'source__author', 'source__reference'
    )
...
class PronounAdmin(TrackedModelAdmin, VersionAdmin):
    ...
    list_filter = (
        'paradigm', 'pronountype', 'paradigm__language', 'paradigm__source',
    )
...
class RelationshipAdmin(TrackedModelAdmin, VersionAdmin):
    date_hierarchy = 'added'
    list_display = (
        'paradigm', 'pronoun1', 'pronoun2', 'relationship', 'comment'
    )
...
class PronounTypeAdmin(TrackedModelAdmin, VersionAdmin):
    list_display = (
        'id', 'sequence', 'person', 'number', 'gender', 'alignment', 'active'
    )
