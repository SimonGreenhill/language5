...
class TestCopyParadigm(DefaultSettingsMixin, TestCase):
    ...
    def test_adds_paradigm_doesnt_affect_previous(self):
        copy_paradigm(self.pdm, self.lang2)
    ...
    def test_adds_rules(self):
        ...
        r1 = Rule.objects.create(
            paradigm=self.pdm, rule="Obey!", editor=self.editor
        )
        ...
        assert newpdm != self.pdm, \
            'new paradigm should not be the same as the old one'
    ...
    def test_adds_relationships(self):
        ...
        assert newrels[0].pronoun1.comment == '0', \
            'Expected 0 got %r' % newrels[0].pronoun1.comment
        assert newrels[0].pronoun2.comment == '1', \
            'Expected 1 got %r' % newrels[0].pronoun2.comment

        assert newrels[1].pronoun1.comment == '0', \
            'Expected 0 got %r' % newrels[0].pronoun1.comment
        assert newrels[1].pronoun2.comment == '2', \
            'Expected 2 got %r' % newrels[0].pronoun2.comment
    ...
    def test_adds_pronouns(self):
        ...
        if Pronoun.objects.count() != (count * 2):
            e = "Should have twice as many pronouns: %r != %r"
            raise AssertionError(e % (count * 2, Pronoun.objects.count()))
        
        # loop over new pronouns and check that they match their ancestral
        # object in all attributes. Remember the ancestral PK is stored in
        # `comment`
        for new_p in newpdm.pronoun_set.all():
            ...
            for attr in ('person', 'number', 'gender', 'alignment'):
                n = getattr(new_p.pronountype, attr)
                o = getattr(old_p.pronountype, attr)
                assert n == o
    ...
    def test_adds_pronoun_entries(self):
        ...
        def _make_token(p):
            return "%s-%s-%s-%s" % (
                p.pronountype.person,
                p.pronountype.number,
                p.pronountype.gender,
                p.pronountype.alignment
            )
