...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding unique constraint on 'Link', fields ['link', 'language']
        db.create_unique('links', ['link', 'language_id'])

        # Adding index on 'Source', fields ['author']
        db.create_index('sources', ['author'])

        # Adding index on 'Source', fields ['year']
        db.create_index('sources', ['year'])

        # Adding index on 'Family', fields ['family']
        db.create_index('families', ['family'])


    def backwards(self, orm):
        # Removing index on 'Family', fields ['family']
        db.delete_index('families', ['family'])

        # Removing index on 'Source', fields ['year']
        db.delete_index('sources', ['year'])

        # Removing index on 'Source', fields ['author']
        db.delete_index('sources', ['author'])

        # Removing unique constraint on 'Link', fields ['link', 'language']
        db.delete_unique('links', ['link', 'language_id'])
    ...
    models = {
        ...
        u'core.language': {
            'Meta': {'ordering': "['language', 'dialect']", 'unique_together': "(('isocode', 'language', 'dialect'),)", 'object_name': 'Language', 'db_table': "'languages'"},
            'added': ('django.db.models.fields.DateTimeField', [], {'auto_now_add': 'True', 'blank': 'True'}),
        ...
        u'core.source': {
            'Meta': {'ordering': "['author', 'year']", 'object_name': 'Source', 'db_table': "'sources'"},
            'added': ('django.db.models.fields.DateTimeField', [], {'auto_now_add': 'True', 'blank': 'True'}),
